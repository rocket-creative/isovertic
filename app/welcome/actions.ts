...
export async function submitOnboarding(_prev: OnboardingState, formData: FormData): Promise<OnboardingState> {
  ...
  const keys = ["company", "website", "contact_name", "role", "audience", "problems", "topics", "competitors", "voice", "social", "newsletter", "claims", "reviewer", "access", "call", "notes"] as const;
  ...
  if (resendKey && to) {
    try {
      await new Resend(resendKey).emails.send({
        ...
        subject: `Baseline intake: ${answers.company} (${planKey || "baseline"})${wantsCall ? " wants a call" : ""}`,
        text: `Email: ${email}\nCheckout: ${sessionId}\n\n${body}`,
      });
    } catch (e) {
  ...
  if (errors.length === 2) {
    ...
    return { ok: false, error: "Something failed on our side. Email georgestoff@rocketcreative.net with your answers and we will take it from there." };
  }
